...
--TASK 1
select C.first_name,C.last_name,(select count(O.id) from orders as O where O.customer_id=C.id and O.payment_type="Cash") as Total_Orders 
from Customers as C 
Group by c.id 
having Total_orders>0;

--TASK 2
select O.id,O.order_date,(select OS.status_name from Orders_Status as OS where OS.ID=O.status_id) as status_name 
from Orders as O where O.id IN ( select order_id from order_details group by order_id having count(product_id)>=3);

--TASK 3
select E.first_name,E.last_name from employees as E where E.id IN ( 
select EP.employee_id from employee_privileges as EP where EP.privilege_id IN ( 
select P.id from privileges as P where P.privilege_name="Purchase Approvals")
) and E.city IN ("Redmond","Bellevue","Kirkland");

--TASK 4
select S.city,count(*) as Total_Suppliers from Suppliers as S group by S.city;

/*ASSUMPTION -for- TASK (4) 
As the cities of all suppliers in the Data are NULL therefore, we get only 1 row when grouped by City and total Suppliers equal to 10 as there are 10 suppliers in the Data.
(If Supplier Table Data had multiple cities then we would have got multiple rows with supplier count in those cities using this Query).*/

--TASK 5
select P.product_name,sum(POD.quantity*POD.unit_cost) as Total_Amount 
from products as P left join purchase_order_details as POD on P.id=POD.product_id 
group by P.id;

/*ASSUMPTION -for- TASK (5)
I have used Left Join instead of simple Join Because it is written in the Question that Show “all” Product names which means those products should also be in the result which were not purchased so their Total Amount will be NULL but they must be present in the result.
Using simple Join will give us only those products names which were purchased therefore, I used Left Join instead of Join.
Furthermore, I have Assumed that the total Amount means Cost not Quantity therefore, for each product I have multiplied unit cost with quantity to get total amount (cost) for each product in every order.
(Otherwise for Quantity we would not have multiplied cost with it we would just sum the quantity of a product in every order to get Total Amount (Quantity)).*/

--TASK 6
select P.Category,min(P.list_price) as Minimum_List_Price,max(P.list_price) as Maximum_List_Price,avg(P.list_price) as Average_List_Price,sum(P.list_price) as Sum_List_Price
from products as P 
group by P.category;

--TASK 7
select E.first_name,E.last_name from Employees as E join orders as O on E.id=O.employee_id join order_details as OD on OD.order_id=O.id group by O.id 
having sum(OD.quantity*OD.unit_price) = (select max(Bill.Total_Amount) from (select sum(OD.quantity*OD.unit_price) as Total_Amount from order_details as OD 
join orders as O on OD.order_id=O.id group by O.id) as Bill);
