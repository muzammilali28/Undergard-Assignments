...
--Q.7
select * from sponsors where sponsor_name != 'azhar' and sponsor_name != 'galib';

--Q.8
select P.fname from players as P
join player_participated as P_P on P.player_id = P_P.player_id
join tornaments as T on P_P.tornaments_id = T.tornaments_id
where T.tornament_name = 'awami';

--Q.9
select * from tornaments where organizer_id IN
(select organizer_id from tornament_organizer where organizer_name = 'alinawaz' or organizer_name = 'shabana azmi');


--Q.10
select fname from players where player_id IN
(select player_id from player_participated where 
(select count(tornaments_id) from tornaments) >= 1 
);

--OR

/*select fname from players where player_id IN
(select player_id from player_participated where tornaments_id IN
(select tornaments_id from tornaments)
);*/

--Q.11
select count(club_id) as NumberOfClubs from chessclub where adress_id IN
(select address_id from address where city='Lahore' or city='Faisalabad');

--Q.12
select P.fname,P.lname from players as P
join player_participated as P_P on P.player_id = P_P.player_id
join tornaments as T on P_P.tornaments_id = T.tornaments_id
join tornament_organizer as T_O on T.organizer_id = T_O.organizer_id
join chessclub as CC on T_O.club_id = CC.club_id
join address as A on CC.adress_id = A.address_id
where T_O.organizer_name = 'Rahat fateh' and A.city = 'lahore';
